...
# To make this step more computationally efficient, large matrix multiplications calculated for the Arnoldi method are performed using CuArrays defined on a GPU.
...
function eigensolve_inflated_generator(𝐆, num_of_Λ, tol, paramtxtfile)
    ...
    # Throughout this function, all CuArrays must be defined with unified memory so that the Arnoldi method doesn't fail when (unavoidable) scalar indexing is performed on these arrays.

    # Convert the inflated generator to a GPU array
    𝔾 = Float64.(cu(𝐆; unified=true))

    # Initialise the Krylov subspace matrix K and the Hessenberg matrix H... 
    K = zeros(size(𝐆, 1), 2*num_of_Λ+1)
    H = zeros(2*num_of_Λ+1, 2*num_of_Λ)

    # ...along with their temporary counterparts K_tmp and Q
    K_tmp = zeros(size(𝐆, 1), 2*num_of_Λ+1)
    Q = zeros(2*num_of_Λ, 2*num_of_Λ)

    # Convert these four matrices to CuArrays
    K_GPU = Float64.(cu(K; unified=true))
    H_GPU = Float64.(cu(H; unified=true))
    K_tmp_GPU = Float64.(cu(K_tmp; unified=true))
    Q_GPU = Float64.(cu(Q; unified=true))

    # Set up a custom ArnoldiWorkspace object containing the previously initialised GPU arrays
    arnoldi_custom = ArnoldiWorkspace(K_GPU, H_GPU; V_tmp = K_tmp_GPU, Q = Q_GPU)

    # Run the partial Schur decomposition component of the Arnoldi method using our GPU arrays
    time_decomp = @elapsed Decomp, History = partialschur!(𝔾, arnoldi_custom; nev=num_of_Λ, which=:LR, tol=tol)

    # Convert Decomp back to the CPU...
    time_convert = @elapsed Decomp_CPU = ArnoldiMethod.PartialSchur(Array(Decomp.Q), Array(Decomp.R), Decomp.eigenvalues)

    # ...and extract the eigenvalues and eigenvectors from Decomp
    time_eig = @elapsed Λ, V = partialeigen(Decomp_CPU)
    ...
    open(paramtxtfile,"a") do file
        println(file,"The time taken to compute $num_of_Λ inflated generator eigenvalues using the Arnoldi Method on the GPU is ",(time_decomp+time_convert+time_eig)," seconds")
    end
